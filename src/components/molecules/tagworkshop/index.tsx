...
type TagWorkshopProps = {
    name: string, desc: string, img: string, islive: boolean, info: {
        date: string,
        duration: number,
        views: number,
        likes: number
...
const MoleculeTagWorkshop: React.FC<TagWorkshopProps> = ({ name, desc, categories, img, info, islive, user }) => {
    return (
        <ContainerTagStyled>
            <ContainerInfoStyled>
                <div>
                ...
                <AtomTitle size="SubTitleLarge" bold>{name}</AtomTitle>
                <div>
                    <span>{info.date}</span>
